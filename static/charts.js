...
function renderTradeSignal(ts) {
    ...
    // Update sidebar trade signal badge
    const sigBadge = document.getElementById('sidebar-signal-badge');
    if (sigBadge) {
        const badgeColors = {
            'BUY': 'bg-green-100 text-green-700',
            'SELL': 'bg-red-100 text-red-700',
            'HOLD': 'bg-yellow-100 text-yellow-700',
            'EXIT_LONG': 'bg-red-200 text-red-800',
            'EXIT_SHORT': 'bg-green-200 text-green-800',
        };
        const shortLabels = { 'BUY': '⬆', 'SELL': '⬇', 'HOLD': '⏸', 'EXIT_LONG': '🚨', 'EXIT_SHORT': '🚨' };
        sigBadge.textContent = shortLabels[ts.action] || '--';
        sigBadge.className = `badge ${badgeColors[ts.action] || 'bg-gray-200 text-gray-500'}`;
    }
}
